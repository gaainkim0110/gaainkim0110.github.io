...
| 화면 크기 | 레이아웃 조정 |
|-----------|--------------|
| Desktop (≥1200px) | 전체 조직도 표시, 가로 스크롤 가능 |
| Tablet (768px~1199px) | 축소 표시, 핀치 줌 지원 |
| Mobile (<768px) | 세로 스크롤 중심, 노드 터치 확대 |
...
├── public/
│   ├── data/
│   │   └── sample_org_data.xlsx    # 샘플 데이터 파일
│   ├── fonts/
│   │   ├── NanumSquareNeo-Regular.woff2
│   │   └── NanumSquareNeo-ExtraBold.woff2
│   └── favicon.ico
│
├── src/
│   ├── app/
...
const nextConfig = {
  ...
  basePath: process.env.NODE_ENV === 'production' ? '/org-chart-manager' : '',
  assetPrefix: process.env.NODE_ENV === 'production' ? '/org-chart-manager/' : '',
};

module.exports = nextConfig;
```

### 7.2 GitHub Actions 워크플로우 (.github/workflows/deploy.yml)
